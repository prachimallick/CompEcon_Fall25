...
@dataclass
class Params:
    # Parameters to ESTIMATE (start near paper Table 3)
    alpha: float = 0.6956
    gamma: float = 0.1331   # adjustment-cost coefficient (paper's γ)
    rho: float = 0.0976
    sigma: float = 0.8932
    phi0: float = 0.0       # fixed costs for external finance
    ...
    # Grid / sim settings (tune for speed/accuracy)
    nK: int = 300
    Kmin: float = 0.01
    Kmax: float = 40.0
    nA: int = 7
...
def solve_dp(params: Params, Kgrid: np.ndarray, 
    ...
    # Kgrid_i (nK, 1), Kprime (1, nK) -> used to build invest = K' - (1-delta)*K
    ...
    # We'll build arrays of shape (nK, nA, nKprime) where axis=2 is choice K'
    for it in range(params.vfi_maxiter):
        # Expected value next period: EV(k',a') will be V(k', a') projected by P_A
        # EV has shape (nK, nA)
        EV = V.dot(P_A.T)   # (nK, nA)
        
        # We need EV evaluated at K' for each K',A  -> later used as EV[k', a]
        # Now compute flow payoff for each current state (K, A) and each choice K'
        # shapes:
        # K_mat: (nK,1), Kprime_mat: (1,nK) -> after broadcast invest shape (nK, nK)
        invest_mat = Kprime_mat - (1.0 - delta) * K_mat   # shape (nK, nKprime) where rows = current K, cols = K'
        # We'll expand invest to (nK, nA, nKprime) by adding A dimension later
        
        # Profit at current K and A (independent of K')
        # pi_mat shape (nK, nA)
        pi_mat = (A_row * (K_mat ** params.alpha))      # (nK, nA)
        
        # Now expand to include K' choices:
        # invest_expanded: (nK, nA, nKprime)
        invest_expanded = invest_mat[:, None, :]        # (nK,1,nKprime) -> broadcasts to (nK,nA,nKprime)
        invest_expanded = np.repeat(invest_expanded, nA, axis=1)  # (nK,nA,nKprime)
        
        # pi_expanded: (nK,nA,1) -> broadcast to (nK,nA,nKprime)
        pi_expanded = pi_mat[:, :, None]
        
        # Adjustment cost: C = 0.5 * gamma * ((I/K)^2) * K  (paper eqn (4))
        safe_K = np.maximum(K_mat, 1e-12)   # (nK,1) safe
        safe_K_exp = safe_K[:, :, None]     # (nK,1,1) -> broadcast
        # compute (I/K) using safe_K broadcasted shape
        # modification in solve_dp (use params.gamma for adjustment cost)
        adj_cost = 0.5 * params.gamma * ((invest_expanded / safe_K_exp) ** 2) * safe_K_exp


        # ensure no negative investments produce nonsense (if negative invest -> disinvestment allowed? adjust as model)
        
        # External finance: ext_funds = max(0, invest - pi)
        ext_funds = np.maximum(0.0, invest_expanded - pi_expanded)   # (nK,nA,nKprime)
        # Fixed cost when ext_funds > 0: phi0 * K_mean (paper uses phi0 relative to mean K)
        finance_cost = params.phi0 * K_mat[:, :, None] * (ext_funds > 0).astype(float)



        # (if you want a variable cost phi1 * ext_funds include it here)
        
        # Flow profit (net): pi - invest - adjustment cost - finance cost
        # note shapes: pi_expanded (nK,nA,nKprime), invest_expanded same, etc.
        net_flow = pi_expanded - invest_expanded - adj_cost - finance_cost
        
        # EV_next: expected continuation value for each K' and A
        # EV has shape (nK, nA). We need EV at chosen K' for each (current K, A, K')
        # That means EV_next[i,j,kprime] = EV[kprime, j]
        # We can index vectorized: create index arrays
        # We'll produce EV_next by broadcasting EV over current K dimension:
        # EV is (nK, nA) -> we want (nK_current, nA, nKprime) where axis2 indexes kprime
        EV_for_Kprime = EV[None, :, :]    # shape (1, nKprime, nA)
        EV_for_Kprime = np.transpose(EV_for_Kprime, (1, 2, 0))  # (nKprime, nA, 1) -> want (1,nA,nKprime)
        EV_next = np.transpose(EV_for_Kprime, (2,1,0))          # (1,nA,nKprime)
        EV_next = np.repeat(EV_next, nK, axis=0)                # (nK, nA, nKprime)
        # (Now EV_next[i,j,k] = EV[k, j])
        ...
        policy_idx_new = np.argmax(RHS, axis=2) # (nK,nA) -> indices of K' choices (0..nK-1)
        ...
        if diff < params.vfi_tol:
            # convert policy_idx to actual capital values if you want
...
def simulate_panel(params: Params, sol: Dict[str, Any],
    ...
    # Precompute grid spacings for K (uniform here, but generalize)
    # For interior central differences we'll use K[k+1] - K[k-1]; for one-sided use K[k+1]-K[k] or K[k]-K[k-1]
    K_forward_diff = np.empty(nK)
    K_backward_diff = np.empty(nK)
    K_forward_diff[:-1] = Kgrid[1:] - Kgrid[:-1]
    K_forward_diff[-1] = K_forward_diff[-2]  # duplicate last step for safety
    K_backward_diff[1:] = Kgrid[1:] - Kgrid[:-1]
    K_backward_diff[0] = K_backward_diff[1]

    for t in range(T_total):
        # Current capital and productivity (vectors)
        ...
        # Compute marginal Q (finite-difference on V over K) — vectorized with boundary handling
        ...
        # handle left boundary (k_idx == 0): use forward difference (V[1] - V[0]) / (K[1]-K[0])
        left_mask = (K_idx == 0)
        if np.any(left_mask):
        ...
        # handle right boundary (k_idx == nK-1): use backward difference (V[nK-1]-V[nK-2])/(K[nK-1]-K[nK-2])
...
def compute_moments(sim_data: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    # Pooled regression: I_t/K_t = a0 + a1 * E_t[q_{t+1}] + a2 * (pi_t/K_t)
    # In simulation we proxy E_t[q_{t+1}] with the realized q at t+1 (common in SMM)
    ...
    # firm-wise AR1 for I/K and take average across firms (more representative)
...
def create_latex_tables(theta_identity: np.ndarray, 
    ...
    # helper to create a LaTeX-safe display name
    def latex_safe(name: str) -> str:
        # Replace literal pi symbol with LaTeX math \pi
    ...
    with open(moments_tex_path, "w", encoding="utf-8") as f:
        ...
        for i in range(n_mom):
            ...
            # Write LaTeX-safe name
            f.write(f"{name} & {target:.4f} & {ident:.4f} & {eff:.4f} & {diff:.4f} \\\\\n")

        f.write("\\bottomrule\n")
        f.write("\\end{tabular}\n")

    # --- CSV summaries (use UTF-8 to be safe) ---
    try:
